    ...
    const submitImport = async () => {
        ...
        try {
            ...
            const response = await axios.post(
                `${API_BASE_URL}/warehouse/import_notes/`,
                { items, notes },
                { headers: { Authorization: `Token ${token}` } }
            );
            ...
            await axios.post(
                `${API_BASE_URL}/warehouse/import_notes/${response.data.id}/complete/`,
                {},
                { headers: { Authorization: `Token ${token}` } }
            );
            ...
            setError(err.response?.data?.error || 'Failed to create import note');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-card rounded-lg p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
                <h2 className="text-2xl font-bold mb-4 text-foreground">Import Stock</h2>
                ...
                {/* Search for variants */}
                <div className="mb-4">
                    <label className="block text-sm font-medium mb-2 text-foreground">Search Product</label>
                ...
                {/* Selected Items */}
                <div className="mb-4">
                    <label className="block text-sm font-medium mb-2 text-foreground">Items to Import ({selectedItems.length})</label>
                ...
                <div className="mb-4">
                    ...
                    <textarea
                        ...
                        placeholder="Supplier, reason, etc."
                ...
                <div className="flex justify-end gap-2">
                    ...
                    >
                        {loading ? 'Processing...' : 'Import Stock'}
                    </button>
                </div>
            </div>
        </div>
        ...
        return (
            <div className="min-h-screen bg-background p-6">
                <div className="text-center py-12">
                    ...
                    <p className="mt-4 text-text-muted">Loading inventory data...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-background p-6">
            <div className="max-w-7xl mx-auto">
                <div className="mb-6 flex justify-between items-center">
                    <h1 className="text-3xl font-bold text-gradient-tet">Inventory Management</h1>
                    <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div className="bg-card p-6 rounded-lg shadow border-l-4 border-blue-500">
                        <div className="text-text-secondary text-sm">Total Variants</div>
                    ...
                    <div className="bg-card p-6 rounded-lg shadow border-l-4 border-orange-500">
                        <div className="text-text-secondary text-sm">Low Stock (≤5)</div>
                    ...
                    <div className="bg-card p-6 rounded-lg shadow border-l-4 border-red-500">
                        <div className="text-text-secondary text-sm">Out of Stock</div>
                        ...
                        <table className="min-w-full divide-y divide-border">
                            <thead className="bg-muted">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">Color / Size</th>
                ...
                <div className="bg-card rounded-lg shadow overflow-hidden">
                    <div className="p-6 border-b border-border">
                        <h2 className="text-xl font-semibold text-foreground">Recent Inventory Transactions</h2>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-border">
                            <thead className="bg-muted">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">Change</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">Stock After</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">Ref ID</th>
                                </tr>
                            </thead>
                            <tbody className="bg-card divide-y divide-border">
                                {recentLogs.length === 0 ? (
